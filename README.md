...
> **📋 Consolidated summary of everything done on 2026-08-08: see [UPDATES-2026-08-08.md](UPDATES-2026-08-08.md)** — full audit results, the PDF-intake root-cause fix, honest meeting errors, Aarav brain improvements, and the new free self-hosted meeting bot.

### 2026-08-08 — Full-flow audit: PDF intake fixed, meeting errors honest, voice verified end-to-end

**Root cause found & fixed — PDF intake never worked.** `services/recommender.py` told the model to "match the IntakeResult schema" without ever showing the schema, so LLaMA invented its own field names and every upload died with *"The model did not return a parseable recommendation."* Fix: an explicit `OUTPUT_FORMAT` JSON skeleton in the system prompt plus a one-shot self-repair retry (validation errors are fed back to the model). `sample-documents/skill_gap_report.pdf` now produces correct drafts (Aisha Osei → Data Privacy Practitioner 90 %, Viktor Lindqvist → ISO 27001 92 %).

**"Meeting mode is not configured yet" was a lie.** The `.env` keys ARE set; the real failures were (1) AgentCall account has **no credits** (402 insufficient credits) and (2) the `loca.lt` tunnel in `AVATHAR_PUBLIC_URL` is dead. Changes: `/meetings/start` now pre-flights the tunnel and returns a precise 503 if it's unreachable; AgentCall 402/401 map to plain-English messages (top-up link / bad key); `api.js` helpers now surface the backend `detail` on ALL requests; Aarav speaks the actual reason instead of "not configured". To actually join meetings: add AgentCall credits + start a fresh tunnel (`cloudflared tunnel --url http://localhost:5180`) + update `AVATHAR_PUBLIC_URL`. Free path unchanged: share screen yourself + ask for the tour. Also `vite.config.js` `allowedHosts` now accepts `.loca.lt`/`.ngrok.app` tunnels, not just cloudflare.

**Aarav's brain tightened.** "Approve X" for someone without a pending request used to make the model waffle ("let me check"); the system prompt now forces approve/reject to always emit the typed action so the deterministic code answers precisely ("Viktor has no pending HIL request right now" + opens HIL). Verified live: overdue-filter navigation, tour intent, garbled-name fuzzy match ("martian chen" → Marcus Chen), approve → read-back → confirm guardrail, junk-audio transcribe fails gracefully.

**Whole pipeline verified against the running stack** (Docker `tn_postgres` + uvicorn :8000 + vite :5180): PDF upload → 2 AI drafts in Studio → submit → pending HIL → Talent Lead approve → assignment active → learner course (3 modules + case study) → modules + case-study done → assignment complete + CIPP/E cert pending. Playwright UI sweep across Talent Lead / Coordinator / Learner personas: **zero console errors**; screenshots confirmed tracker voice-filtering, Studio draft editor, learner picker.

**Gotcha:** `npm run dev -- --port 5180` mangles the flag (vite starts on 5173); use `npx vite --port 5180 --strictPort`.

### 2026-08-08 — Own self-hosted meeting bot (free AgentCall replacement)

Built `/ownbot` — our own meeting bot using the same technology AgentCall uses, running locally at zero cost:

- **How it works**: Playwright launches a visible Chrome that joins the Teams/Meet/Zoom call as a guest named "Aarav". Before the meeting page loads, an injected script overrides `getUserMedia` so the bot's **webcam is a live animated avatar canvas** (blinking, mouth synced to speech) and its **microphone is a WebAudio destination** fed by **free Microsoft Edge neural TTS** (`en-IN-PrabhatNeural`). It **hears the meeting** by hooking `RTCPeerConnection` and tapping every remote audio track; a VAD segments utterances which are transcribed by **Groq Whisper** (key already in `.env`). Transcripts run through the **same Aarav brain** (trigger word → command → HIL read-back → "confirm") as the in-app assistant and the old AgentCall bridge, driving the shared screen via the existing `/?present=1` present-state. Screenshare = a second tab pinned to the title "Aarav Presents" (auto-picked by Chrome capture flags via `POST /ownbot/share`).
- **Files**: `backend/services/meetbot.py` (browser bot + media injection), `backend/routers/ownbot.py` (start/status/share/say/stop + brain bridge). Frontend 📹 button now tries the own bot first and falls back to AgentCall.
- **Verified end-to-end** against a fake-meeting harness (real WebRTC loopback carrying synthesized speech): bot's fake cam/mic accepted, remote track tapped, *"Aarav, please show the overdue assignments"* was heard → transcribed → present-state switched to tracker/overdue → spoken reply. Chrome quirk handled: remote tracks only feed WebAudio when attached to a (muted) `<audio>` element.
- **Requirements**: `pip install playwright edge-tts` + `python -m playwright install chromium` (done on this machine; added to requirements.txt).
- **Honest limitations**: the bot's Chrome window must stay open on this machine; someone must **admit it from the lobby**; Teams/Meet join-screen selectors change occasionally (the join flow is best-effort with fallbacks — if it stalls, finish the join by hand in the bot's window, everything else still works); Zoom web join is often blocked by meeting settings; no per-speaker diarization; no tunnel needed at all.
- **Demo flow**: 📹 → paste meeting link → admit "Aarav" from the lobby → *"Aarav, give us a tour"* / *"Aarav, show the overdue assignments"* / *"Aarav, approve Chen Wei's request"* → *"confirm"*. `POST /ownbot/share` (or click Share in the bot window → "Aarav Presents" tab) to put the app on screen.

### 2026-08-08 (follow-up) — Own-bot polish after first real use

User's first real 📹 click actually WORKED — the bot joined their Google Meet and sat in the lobby waiting to be admitted; a second click then hit "409 already running" and confusingly cascaded into the AgentCall credits error. Fixes: `/ownbot/start` now auto-replaces a running bot (old Chrome window closes, new call joins — no more 409); Aarav's chat now handles bot controls locally — "stop the bot" / "leave the meeting" / "delete" stops it, "bot status" reports it — instead of sending those to the LLM. **Remember: after clicking 📹, the bot appears in the MEETING'S lobby — click Admit there.**

### 2026-08-08 (follow-up) — Fresh tunnel for meeting mode

The dead `loca.lt` tunnel was replaced: started `cloudflared tunnel --url http://localhost:5180`, set `AVATHAR_PUBLIC_URL=https://committed-fleece-industrial-asking.trycloudflare.com` in `backend/.env`, restarted the backend. The tunnel now serves `/aarav-cam` (HTTP 200) and `/meetings/start` passes the tunnel pre-flight. **Sole remaining blocker for bot mode: the AgentCall account has no credits** (top up at app.agentcall.dev/add-credits). Remember: quick-tunnel URLs die when cloudflared stops — restart it and update `.env` before each demo.
